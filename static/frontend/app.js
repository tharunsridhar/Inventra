...
// Django's DRF routers require a trailing slash on every router-generated
// URL (list, detail, and @action endpoints alike); this frontend was
// originally written against FastAPI, which had no such rule. Rather than
// rewrite every one of the ~30 call sites below, every request funnels
// through here, so the exemptions list is the only place that needs to
// know which paths are Django's own hand-written path() routes (already
// slash-free) versus DefaultRouter-generated ones (always need one).
const NO_TRAILING_SLASH = new Set([
  "/auth/login", "/auth/register", "/auth/refresh", "/auth/logout",
  "/users/me", "/damage", "/dashboard",
  "/reports/products", "/reports/purchases", "/reports/sales", "/reports/inventory",
]);

function withTrailingSlash(path) {
  if (path.endsWith("/") || NO_TRAILING_SLASH.has(path)) return path;
  return path + "/";
}

async function api(path, { method = "GET", body, auth = true, query } = {}) {
  let url = API_BASE + withTrailingSlash(path);
  if (query) {
